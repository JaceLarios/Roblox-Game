...
def mat(name,hex,kind='Enamel',metal=.25,rough=.35,glow=0):
 ...
 texturepath=OUT.parent/'new-addons/textures'/f'{kind}_RoughnessMap.png'
...
def loft(name,stations,ma):
 ...
 o=mesh(name,verts,faces,ma);b=o.modifiers.new('Crisp panel bevel','BEVEL');b.width=.025;b.segments=2;o.modifiers.new('Body normals','WEIGHTED_NORMAL');return o
...
def arch(x,y,z,r,w,ma):
 ...
 for i,f in enumerate(o.data.polygons):f.use_smooth=i//(N-1) in [1,3]
def seat(x,y,z,w=.55):
...
def bumper(y,z,w):
 box('Bumper impact bar',(0,y,z),(w,.16,.19),'Chrome',.07)
 for x in [-w*.28,w*.28]:box('Bumper rubber overrider',(x,y-.08,z),(.11,.10,.25),'Rubber',.022)
def spring(a,b,r=.09):
...
model('Rusted Sedan')
box('Kart floor pan',(0,.15,.43),(1.62,2.82,.15),'Iron',.06)
for y in [-1.22,1.26]:
 axle(y,.49,1.12)
...
for x in [-.67,.67]:headlamp(x,-1.74,1.015,.135)
bumper(-2.02,.57,1.65)
for x in [-.93,.93]:
...
box('Wide upholstered seat base',(0,.22,.80),(1.08,.74,.24),'Seat',.11)
